...
class DashboardView(QWidget):
    ...
    def setup_metrics_row(self):
        ...
        metrics = [
            ("91", "Components", "📚 Open Library", lambda: self.window().switch_view(1)),
            ("29", "Lessons", "🎓 Open syllabus", lambda: self.window().navigate_to_lesson("passive_resistors")),
            ("12", "Simulators", "🧪 Run simulations", lambda: self.window().navigate_to_simulation(0)),
            ("11", "Calculators", "🛠 Open calculators", lambda: self.window().navigate_to_calculator(0)),
            ("270+", "Q&A Questions", "🎓 Test knowledge", lambda: self.window().switch_view(4)),
            ("60+", "Formulas", "🛠 Calculate formulas", lambda: self.window().navigate_to_calculator(0))
        ]
        for val, label, tooltip, click_fn in metrics:
            card = QFrame(); card.setObjectName("metric-card"); card.setStyleSheet("QFrame#metric-card { background-color: #141B2D; border: 1px solid #26334D; border-radius: 10px; padding: 12px; } QFrame#metric-card:hover { border-color: #06B6D4; background-color: #1B2740; }"); card.setCursor(Qt.CursorShape.PointingHandCursor); card.mousePressEvent = lambda event, fn=click_fn: fn()
    ...
    def setup_workspace_launcher(self):
        widget = QWidget(); layout = QVBoxLayout(widget); layout.setContentsMargins(0, 0, 0, 0); layout.setSpacing(10); header = QLabel("Quick Launch Workspace"); header.setStyleSheet("color: #06B6D4; font-size: 11pt; font-weight: bold; letter-spacing: 0.5px;"); layout.addWidget(header); grid = QGridLayout(); grid.setSpacing(15); shortcuts = [("Component Library", "📚 Browse electronic parts.", "fa5s.book", lambda: self.window().switch_view(1)), ("Analog Electronics Hub", "⚡ Theory, Calculators, Transient Solvers & Virtual DSO Oscilloscope.", "fa5s.wave-square", lambda: self.window().switch_view(2)), ("Digital System Design Hub", "⚡ Digital Electronics Learning Hub: Theory, Sequential Path, Breadboard Trainer & IC Matrix.", "fa5s.microchip", lambda: self.window().switch_view(3)), ("Grand Viva Board", "🎓 Core viva and interview prep.", "fa5s.user-graduate", lambda: self.window().switch_view(4)), ("Engineering Toolkit", "🛠 Quick Ohm's law, reactance & time constant calculators.", "fa5s.tools", lambda: self.window().navigate_to_calculator(0)), ("Simulation Lab & Oscilloscope", "🧪 Real-time physical solvers and DSO.", "fa5s.flask", lambda: self.window().navigate_to_simulation(0))]
        for i, (name, desc, icon, click_fn) in enumerate(shortcuts):
            card = QFrame(); card.setObjectName("launch-card"); card.setStyleSheet("QFrame#launch-card { background-color: #141B2D; border: 1px solid #26334D; border-radius: 10px; padding: 15px; } QFrame#launch-card:hover { border-color: #06B6D4; background-color: #1B2740; }"); card.setCursor(Qt.CursorShape.PointingHandCursor); card.mousePressEvent = lambda event, fn=click_fn: fn(); c_layout = QHBoxLayout(card); c_layout.setContentsMargins(10, 10, 10, 10); c_layout.setSpacing(15); icon_lbl = QLabel(); icon_lbl.setPixmap(qta.icon(icon, color="#06B6D4").pixmap(24, 24)); c_layout.addWidget(icon_lbl); txt_layout = QVBoxLayout(); txt_layout.setSpacing(3); name_lbl = QLabel(name); name_lbl.setStyleSheet("font-weight: bold; color: #FFFFFF; font-size: 11pt;"); desc_lbl = QLabel(desc); desc_lbl.setStyleSheet("color: #C9D1E3; font-size: 9pt;"); desc_lbl.setWordWrap(True); txt_layout.addWidget(name_lbl); txt_layout.addWidget(desc_lbl); c_layout.addLayout(txt_layout); grid.addWidget(card, i // 3, i % 3)
