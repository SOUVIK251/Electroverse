...
class HalfWaveRectifierSimulation(BaseSimulation):
    """Simulator for Half-Wave Diode Rectification with rolling time waveform."""
    
    def __init__(self, parent=None):
        super().__init__(
            title="Half-Wave Rectifier Simulator",
            formula_str="Vout(t) = max(0, Vin(t) - Vd)",
            explanation_str=(
                "A half-wave rectifier uses a single diode to convert AC voltage to pulsating DC. "
                "The diode only conducts when forward-biased (Vin > Vd), clipping the entire negative "
                "half-cycle. The peak output voltage is reduced by the diode forward drop (Vd ≈ 0.7V for Silicon)."
            ),
            parent=parent
        )
    ...
    def reset_data(self):
        ...
        self.vin_points = []
        self.vout_points = []
        self.visualizer.set_ac_voltage(0.0, "half")
    ...
    def update_simulation_step(self, dt: float):
        ...
        # Update visualizer (conduction path)
        self.visualizer.set_ac_voltage(vin, "half")
        self.update_plot_canvas()
        ...
        self.steps_text.setText(
            f"**Half-Wave Rectifier Status**:\n\n"
            f"- **Input Frequency**: {format_eng(base_freq, 'Hz')}\n"
            f"- **Diode Voltage Drop**: {format_eng(base_vd, 'V')}\n"
            f"- **AC Peak Voltage**: {format_eng(base_vac, 'V')}\n"
            f"- **DC Peak Output**: {format_eng(max(0.0, base_vac - base_vd), 'V')}"
        )
    ...
    def update_plot_canvas(self):
        ...
        self.plot_canvas.ax.set_title("Half-Wave Rectification Waveforms")
    ...
    def get_oscilloscope_waveforms(self, time_axis):
        ...
        ch2 = np.maximum(0.0, ch1 - base_vd)
