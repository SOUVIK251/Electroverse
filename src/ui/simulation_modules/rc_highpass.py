...
class RCHighPassSimulation(BaseSimulation):
    """Simulator for RC High-Pass filter frequency sweep with animated frequency dot."""
    
    def __init__(self, parent=None):
        super().__init__(
            title="RC High-Pass Filter Frequency Sweeper",
            formula_str="H(f) = (2π × f × R × C) / √(1 + (2π × f × R × C)²)   |   fc = 1 / (2π × R × C)",
            explanation_str=(
                "A high-pass filter blocks signals below its cutoff frequency (fc) and passes "
                "signals above it. At fc, the output voltage is 70.7% of the input voltage (-3dB). "
    ...
    def init_simulation(self):
        self.plot_canvas.ax.set_title("High-Pass Frequency Response (Bode Plot)")
    ...
    def reset_data(self):
        self.sim_time = 0.0
        self.current_f = 1.0
        self.visualizer.set_voltages(5.0, 0.0)
    ...
    def update_simulation_step(self, dt: float):
        ...
        f_swept = 10.0 ** (t_ratio * 5.0)

        # Gain H(f)
        omega_rc = 2.0 * np.pi * f_swept * base_r * base_c
        h = omega_rc / np.sqrt(1.0 + omega_rc ** 2)
    ...
    def update_plot_canvas(self):
        ...
        # Plot full bode curve
        f_axis = np.logspace(0, 5, 300)
        omega_rc_axis = 2.0 * np.pi * f_axis * base_r * base_c
        h_axis = omega_rc_axis / np.sqrt(1.0 + omega_rc_axis ** 2)
        ...
        omega_rc_swept = 2.0 * np.pi * f_swept * base_r * base_c
        h_swept = omega_rc_swept / np.sqrt(1.0 + omega_rc_swept ** 2)
        ...
        # Shaded stopband and passband (High Pass)
        self.plot_canvas.ax.axvspan(1.0, fc, facecolor="#ef4444", alpha=0.08, label="Stopband")
        self.plot_canvas.ax.axvspan(fc, 100000.0, facecolor="#10b981", alpha=0.12, label="Passband")
        ...
        self.plot_canvas.ax.set_title("RC High-Pass Bode Frequency Sweep")
