...
class DigitalLogicEngine:
    """Single Source of Truth: Evaluates all 7 fundamental logic gates."""

    @classmethod
    def evaluate_gate(cls, gate_type: str, inputs: list) -> int:
        """Solves boolean logic for AND, OR, NOT, NOR, NAND, XOR, XNOR."""
        gt = gate_type.upper()
        if "XNOR" in gt:
...
class DSDLabView(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.current_gate_idx = 0  # Gate 1 to 4
    ...
    def init_ui(self):
        ...
        # 2. Wire Color Palette & Eraser Tool Swatch Bar
        color_bar = self.create_wire_color_palette()
        main_layout.addWidget(color_bar)
        
        # 3. TOP 3-COLUMN HEADER ROW (Power & Input Block | Next Connection Card | Output Block)
        ...
        # 4. MAIN WORK SPLITTER (Left Palette | Full-Width Breadboard | Right Inspector)
    ...
    def create_next_connection_card_panel(self):
        ...
        self.lbl_card_from = QLabel("+5V Power Rail")
    ...
    def create_left_toolbox(self):
        ...
        self.ic_selector.addItem("IC 7447 (BCD to 7-Segment)", "7447")
        self.ic_selector.addItem("IC 7483 (4-Bit Binary Adder)", "7483")
        self.ic_selector.addItem("IC 74138 (3-to-8 Decoder)", "74138")
        self.ic_selector.addItem("IC 74151 (8:1 MUX)", "74151")
        self.ic_selector.addItem("IC 74153 (Dual 4:1 MUX)", "74153")
    ...
    def build_2d_breadboard_grid(self):
        ...
        # 1. Top Power & 8 Input (A-H) / 4 Output (Y0-Y3) Sockets
        ...
        # Top Power Rails (+5V Red & GND Black)
        for col in range(30):
            x = 25 + (col * 16)
        ...
        # Top Terminal Strips (Columns A-E, Rows 1-30)
        for col in range(30):
            x = 25 + (col * 16)
        ...
        # Bottom Power Rails (+5V Red & GND Black)
        for col in range(30):
            x = 25 + (col * 16)
        ...
        # 3. REALISTIC DIP IC PACKAGE PLACEMENT
        ...
        # Top Row (Pins 1 to half_pins plugging into Row E, y=164)
        for i in range(half_pins):
            pin_num = i + 1
            col_idx = 10 + i
            hole_x = 25 + (col_idx * 16)
            hole_y = 164  # Terminal Row E
        ...
        # Bottom Row (Pins pins_cnt down to half_pins+1 plugging into Row F, y=198)
        for i in range(half_pins):
            pin_num = pins_cnt - i
            col_idx = 10 + i
            hole_x = 25 + (col_idx * 16)
            hole_y = 198  # Terminal Row F
    ...
    def update_metadata_guided_flow(self):
        """Dynamically resolves guided wiring steps connecting Power Terminals -> Rails -> IC Pins -> Output Terminals."""
        ic_info = ECKBLoader.get_ic(self.current_ic_key) or {}
    ...
    def solve_circuit_logic(self):
        """Dynamic Graph Logic Solver: Evaluates active IC gate & routes output to connected Y0-Y3 terminals."""
